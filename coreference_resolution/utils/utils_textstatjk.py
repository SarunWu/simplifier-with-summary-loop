...
class textstatistics:
    ...
    @lru_cache(maxsize=128)
    def dcdifficult_words(self, text: str) -> int:
        ...
        """
        return len(self.dcdifficult_words_list(text))

    @lru_cache(maxsize=128)
    def dcdifficult_words_list(
                self, text: str
            ) -> List[str]:
    ...
    @lru_cache(maxsize=128)
    def dcis_easy_word(self, word: str) -> bool:
        return not self.dcis_difficult_word(word)

    @lru_cache(maxsize=128)
    def dcdale_chall_readability_score(self, text: str) -> float:
        ...
        count = word_count - self.dcdifficult_words(text)
